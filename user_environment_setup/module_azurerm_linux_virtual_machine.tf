module "module_azurerm_linux_virtual_machine" {
  ...
  name = format("vm-%s-%s", each.value.username, each.value.suffix)
  ...
  computer_name  = format("vm-%s-%s", each.value.username, each.value.suffix)

  disable_password_authentication = false

  os_disk_name                 = format("disk-os-%s-%s", each.value.username, each.value.suffix)
