module "module_azurerm_storage_account" {
  ...
  name                     = length(replace(format("%s%s%s", each.value.username, random_id.id[each.value.username].hex, each.value.suffix), "-", "")) > 24 ? substr(replace(format("%s%s%s", each.value.username, random_id.id[each.value.username].hex, each.value.suffix), "-", ""), 0, 24) : replace(format("%s%s%s", each.value.username, random_id.id[each.value.username].hex, each.value.suffix), "-", "")
